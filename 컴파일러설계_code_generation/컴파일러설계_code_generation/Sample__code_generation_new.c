// 주의: 프로그램에서  |||<<< 가 나온 부분부터  >>>|||  까지가 지워진 부분임.
// 여기를 채워서 사용해야 함.
...
typedef struct tkt { // 하나의 토큰을 나타내는 구조체.
	...
	char sub_kind[3];  // relop 토큰인 경우 다시 구분하는데 이용됨. NUM 토큰의 양의 정수인지("in") 실수인지를("do") 구분.
	int sub_data; // ID 토큰의 심볼테이블엔트리 번호, 양의 정수 값을 저장함.
	double rnum;  // 실수인 경우 그 값을 가짐
	char str[30]; // 토큰을 구성하는 스트링. 
} tokentype;
...
int total_keywords = 19; // keyword 수가 달라지면 여기를 수정하여 주어야 함.
...
typedef struct ssym {
	 ...
	 char str[30]; // string of the symbol
...
typedef struct struc_tree_node { // 
	...
	char place[20], begin[12], next[12], tru[12], fal[12]; // attributes
	char* code;  // attributes
}  ty_tree_node;
...
int main()
{
	...
	// 파일의 소스프로그램에 대한 파싱을 수행. 파스트리를 만듬.
	...
	// Root_parse_tree 가 가리키는 파스트리는 기존시작 비단말기호가 루트노드임.
	// Augmented rule 에 의거하여 새로운 시작비단말 기호를 가지는 새로운 루트를 위에 올려줌.
	...
	fclose(fpc);
	printf("Code has been stored in the file: code.txt.\n");
...
void code_gen(ty_ptr_tree_node cur) {
	...
	switch (cur->rule_no_used) {
		...
		case 5: // S -> if ( BE ) then S1 else S2
			...
			strcpy(S2->begin, BE->fal ); strcpy(S2->next, S->next);   //*** 여기 BE->fal
			code_gen(BE);
			code_gen(S1);  // *** 여기 S1
			code_gen(S2);
		...
		case 6: // S -> while ( BE ) do S1
			...
			strcpy(S1->next, S->begin); // *** 여기 S->begin
			code_gen(BE); 
			code_gen(S1);
		...
		case 7: // E-> T
			...
			break;
		case 8: // E -> E1 + T
			E = cur; E1 = cur->children[0];    T = cur->children[2]; // *** 요기 children[2]
		...
		case 13:// F -> id
			...
			break;
		case 14: // F -> num
			F = cur; num_t = cur->children[0] ; // *** 요기 childlren[0]
		...
		case 15: // F -> ( E )
			...
			code_gen(E);
			strcpy(F->place, E->place ); // *** 요기 E->place
		...
		case 17: // BE -> BE1 || BT
			...
			strcpy(BE1->tru, BE->tru ); strcpy(BE1->fal, newlabel());  /// *** 요기  BE->tru
			strcpy(BT->tru, BE->tru); strcpy(BT->fal, BE->fal);
			code_gen(BE1);  // *** 요기 BE1
			code_gen(BT);
		...
		case 22: // BF -> E1 rop E2
			BF = cur; E1 = cur->children[0]; rop = cur->children[1]; E2 = cur->children[2]; // *** 요기 cur->children[0]
			code_gen(E1);
			code_gen(E2);
			BF->code = strcat_my2(E1->code, E2->code, "if ", E1->place , " ", rop->nodesym.str, " ", // *** 요기 place
				                     E2->place , " goto ", BF->tru, "\ngoto ", BF->fal ); // *** 요기 place , fal
			strcat(BF->code, "\n"); // this is needed.
			break;
		case 23: // BF -> ( BE )
		...
		case 24: // BF -> ! BF1
			...
			strcpy(BF1->tru, BF->fal); strcpy(BF1->fal, BF->tru ); // *** 요기 BF->tru
			code_gen(BF1);
...
sym get_next_token(FILE * fps)  {
	...
	if (a_tok.kind == EOF_TOK) {
		token.no = Max_terminal - 1; // this is a number of terminal $.
